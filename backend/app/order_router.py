...
async def create_an_order(   
    ...
    items = await db.execute(select(CartItem).where(
    ...
    for order in previous_orders:
        order_items_ids = (item.product_id for item in order.order_items)
...
async def get_all_orders_for_a_user(
    ...
    order_history = await db.execute(select(Order).where(Order.user_id == current_user.id))
