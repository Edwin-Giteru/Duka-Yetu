...
from backend.app.db.schemas.payment import PaymentRequest, PaymentResponse
from sqlalchemy.orm import Session
from backend.app.Controllers.auth.dependencies import get_db
from backend.app.db.model.models import Order, OrderStatus, Payment, OrderPayment
import requests
...
async def initiate_payment(
    ...
    try:
        order = db.query(Order).filter(Order.id == payment_request.order_id, Order.user_id == current_user.id).first()
        if not order:
